...
-(void)downloadWithDlUrl:(NSString *)inDLUrl savePath:(NSString *)DLSavePath mode:(NSString *)inMode{
	...
	NSLog(@"savapath = %@ and temppath = %@",DLSavePath,tempPath);
	[udf setValue:DLSavePath forKey:dPathKey];
	[udf synchronize];
	int mode = [inMode intValue];
    NSLog(@"hui-->downloadWithDlUrl-->1-->inDLUrl =%@",inDLUrl);
    ...
    if (NSNotFound != range_.location) {
        ...
        NSLog(@"hui-->downloadWithDlUrl-->2-->inDLUrl =%@",inDLUrl);
    }
...
-(void)request:(ASIHTTPRequest *)request didReceiveResponseHeaders:(NSDictionary *)responseHeaders{
    ...
    //	NSLog(@"hui-->uexDownLoadMgr-->filesize = %lld",fileTotalLength);
...
-(void)requestFailed:(ASIHTTPRequest *)request{
    //保存现场
    NSLog(@"hui-->requestFailed-->request error =%@",[request error]);
	NSUserDefaults *udf = [NSUserDefaults standardUserDefaults];
	NSString *urlstr = [request.userInfo objectForKey:@"reqUrl"];
